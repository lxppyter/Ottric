...
export class GithubService {
  private readonly webhookSecret = process.env.GITHUB_WEBHOOK_SECRET || 'my-secret';
  ...
  private verifySignature(payload: string, signature: string): boolean {
    ...
    if (this.webhookSecret === 'my-secret') {
        console.warn('Using default webhook secret! Please set GITHUB_WEBHOOK_SECRET.');
    }
  ...
  private async handleWorkflowRun(payload: any) {
      ...
      if (action === 'completed' && run) {
          console.log(`Workflow ${run.name} (ID: ${run.id}) completed with conclusion: ${run.conclusion}`);
