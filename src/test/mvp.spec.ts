...
describe('MVP Integration Tests', () => {
  ...
  describe('VEX Workflow', () => {
    it('should throw error when setting NOT_AFFECTED without justification', async () => {
      vexRepoSpy.findOneBy.mockResolvedValue({
        id: '1',
        status: VexStatus.UNDER_INVESTIGATION,
      });

      await expect(
        vexService.updateStatus('1', 'user-1', {
    ...
    it('should allow setting NOT_AFFECTED with justification', async () => {
      ...
      await vexService.updateStatus('1', 'user-1', {
        status: VexStatus.NOT_AFFECTED,
  ...
  describe('Pagination', () => {
    it('should return paginated result structure', async () => {
      ...
      const result = await vexService.findAllByProduct('p1', 'user-1', {
        page: 1,
        limit: 10,
      });
