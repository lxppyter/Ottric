...
import { NotificationChannel, NotificationType } from './entities/notification-channel.entity';
...
export class NotificationsService {
  ...
  async createChannel(userId: string, data: { name: string; type: NotificationType; config: any; triggers: string[] }) {
  ...
  async deleteChannel(userId: string, channelId: string) {
      ...
      const channel = await this.channelRepository.findOne({ where: { id: channelId }, relations: ['organization'] });
      
      if (!channel) throw new NotFoundException("Channel not found");
      if (channel.organization.id !== org.id) throw new NotFoundException("Channel not found"); // Security check
  ...
  async dispatch(userId: string, event: string, payload: any) {
      ...
      for (const channel of channels) {
          try {
              console.log(`[Dispatcher] Sending to channel: ${channel.name} (${channel.type})`);
              ...
              if (channel.type === 'SLACK' || channel.type === 'WEBHOOK') {
                  ...
                  // Mock Email Sending
                  console.log(`[Dispatcher] 📧 MOCK EMAIL sent to ${channel.config?.email}`);
                  console.log(`Subject: Security Alert - ${event}`);
                  console.log(`Body:`, message.text);
              }

          } catch (error) {
              console.error(`[Dispatcher] Failed to send to channel ${channel.name}:`, error.message);
          }
      }
  }
}
