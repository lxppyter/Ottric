import { Controller, Post, Get, Delete, Param, UseGuards, Req, Body, Patch, UnauthorizedException } from '@nestjs/common';
...
export class OrganizationController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create Organization', description: 'Create a new organization and return refreshed token.' })
  ...
  @ApiOperation({ summary: 'Generate Invitation Key', description: 'Generate a new generic invitation key (valid for 15 mins). Admin only.' })
  ...
  @Post('api-keys')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Generate API Key', description: 'Generate a new API key for CI/CD integration. Admin only.' })
  ...
  @Get('api-keys')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'List API Keys', description: 'List active API keys. Admin only.' })
  ...
  @Get('invites')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'List Invites', description: 'List pending invites. Admin only.' })
  listInvites(@Req() req) {
  ...
  @Patch('me')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Update Organization', description: 'Update organization details. Admin only.' })
  updateOrganization(@Req() req, @Body() body: { name: string }) {
      if (!req.user.organizationId) {
          throw new UnauthorizedException("User not in organization");
      }
      return this.usersService.updateOrganizationName(req.user.organizationId, body.name);
  }

  @Delete('api-keys/:id')
  @Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Delete API Key', description: 'Revoke an API key. Admin only.' })
  async deleteApiKey(@Req() req, @Param('id') id: string) {
      return this.usersService.deleteApiKey(req.user.id, id);
  }

  @Delete('invites/:id')  
  @ApiOperation({ summary: 'Delete Invitation', description: 'Delete an invitation key.' })
  ...
  @Get('members')
  @ApiOperation({ summary: 'List Members', description: 'List members of the organization.' })
